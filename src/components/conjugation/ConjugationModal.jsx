...
const ConjugationModal = ({ entry, onClose }) => {
  ...
  const availableBinyanim = useMemo(() => {
    if (!entry) return [];
    return getAvailableBinyanim(entry.binyanim || [], entry.isAramaic, entry.isHebrew);
  }, [entry]);
  ...
  const langLabel = entry.isHebrew ? 'Hebrew' : 'Aramaic';

  return (
    <div
      className={`fixed inset-0 z-[70] bg-white dark:bg-gray-900 flex flex-col transition-transform duration-300 ease-in-out ${
      ...
      <div className="sticky top-0 z-10 flex items-center px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-pink-600 to-pink-700">
        ...
        <div className="text-center flex-1">
          <h2 className="text-white font-bold text-lg">
            {selectedBinyan ? availableBinyanim.find(b => b.key === selectedBinyan)?.name : `${langLabel} Conjugation`}
          </h2>
          ...
          <div>
            ...
            <div className="mt-4 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50">
              <p className="text-xs text-amber-800 dark:text-amber-200">
                ...
                Weak roots and irregular verbs may have different forms in actual usage.
              </p>
            </div>
          </div>
        ) : (
